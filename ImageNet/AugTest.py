...
for i, data in enumerate(seed_dataloader, 0):
    ...
    for search in range(max_search):
        ...
        for iterate in range(T): 
            ...
            #初始化就已经是对抗样本
            if pred_label_random[0][0]!= orig_label0[0]:
                #print('Is a random adv!')
                gen_img = x.clone()
                ...
                fea = feature_Extract_net(gen_img)
                ...
                test1 = pre_model_test1(gen_img)
                _,test1_pre = torch.topk(test1, 1)
                test_la1 = test1_pre[0][0]
                
                test2 = pre_model_test2(gen_img)
                ...
                adv_img_save.save('{}adv_{}_{}_{}.jpg'.format(save_dir,str(num_adv),str(orig_label0[0].cpu().numpy()),str(pred_label_random[0][0].cpu().numpy())))
            ...
            if pred_label_0[0][0]!= orig_label0[0]:
                ...
                fea = feature_Extract_net(gen_img_copy)
                feat = inter_feature['features[28]'] 
                feat = feat.cpu()
                div = geo_(feat)
                diversity = div.clone()
