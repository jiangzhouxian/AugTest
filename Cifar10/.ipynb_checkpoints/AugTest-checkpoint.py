...
from torchvision.models import resnet18,resnet50,wide_resnet50_2,vgg16
...
if args.model == 0:
    ...
    print("*****Test for ResNet18********")
elif args.model == 1:
    subdir = 'Geometric_ResNet50'
    pre_model_test1 = r18(rs18)
    pre_model_test1.load_state_dict(torch.load('./Models/cifar10_resnet18.pth'))
    pre_model = r50(rs50)
    pre_model.load_state_dict(torch.load('./Models/cifar10_resnet50.pth'))
    pre_model_test2 = wr50(wrs50)
    pre_model_test2.load_state_dict(torch.load('./Models/cifar10_wideresnet50.pth'))
    ...
    pre_model_test1 = r18(rs18)
    pre_model_test1.load_state_dict(torch.load('./Models/cifar10_resnet18.pth'))
    pre_model_test2 = r50(rs50)
    pre_model_test2.load_state_dict(torch.load('./Models/cifar10_resnet50.pth'))
    pre_model = wr50(wrs50)
    pre_model.load_state_dict(torch.load('./Models/cifar10_wideresnet50.pth'))
        ...
        for iterate in range(T): 
            ...
            for bat in range(batch):
                ...
                if pred_label[0]!= orig_label[bat]:
                    ...
                    num_adv+=1
                    if bat == 0:
                        gen_img_clone = gen_img[bat].clone()
                        gen_img_clone = gen_img_clone.detach().numpy()
                        gen_data_0.append(gen_img_clone)
                    elif bat == 1:
                        gen_img_clone = gen_img[bat].clone()
                        gen_img_clone = gen_img_clone.detach().numpy()
                        gen_data_1.append(gen_img_clone)
                    ...
                    adv_img_save.save('{}adv_{}_{}_{}.jpg'.format(save_dir,str(num_adv),str(orig_label[1].numpy()),str(pred_label[0].numpy())))
